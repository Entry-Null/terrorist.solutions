...
local CrimTab  = Window:AddTab("Criminality")
local CRIMBOX = CrimTab:AddLeftTabbox("Main")
local Crim = CRIMBOX:AddTab("Main")
...
game:GetService("Players").LocalPlayer.PlayerGui.ChildAdded:Connect(function(child)
    if Toggles.AutoPick.Value then
        if child.Name == "LockpickGUI" then
            if child:WaitForChild("MF") then
                child.MF.LP_Frame.Line.Size = UDim2.new(1, 0, 0, 1000)
                child.MF.LP_Frame.Line.ImageTransparency = 1
            end
        end
    end
end)

Crim:AddToggle("AutoPick", {Text = "Auto Lockpick", Default = false})

Crim:AddButton("Crim Admin Notification", function ()
...
function trashtalkloop()
    while wait() do
        if Options.trashtalkheld.Toggled then
            if not db then
                db = true
                realChat(trashtalktbl[math.random(#trashtalktbl)])
                wait(3)
                db = false
            end
        end
...
BoomboxHV:AddSlider("BoomBoxRadius", {Text = "Visualise Radius", Min = 0, Max = 10, Default =  1, Rounding = 0, Suffix = 'studs'})
...
Sound:AddSlider("FootstepsVolume", {Text = "Step Volume", Min = 0, Max = 10, Default = config['FootstepsVol'] or 1, Rounding = 0, Suffix = 'db'})
...
Main:AddSlider("HeadhitChance", {Text = "Headshot Chance", Min = 0, Max = 100, Default = config['headShotChance'] or 30, Rounding = 0, Suffix = '%'})
...
TriggerHolder:AddSlider("TriggerMSDelay", {Text = "Delay in MS", Min = 0, Max = 2000, Default = config['TriggerBotMSDelay'] or 30, Rounding = 0, Suffix = 'ms'})
